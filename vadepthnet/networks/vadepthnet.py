...
from .loss import VarLoss, SILogLoss, VarFlowLoss, WindowVarLoss
########################################################################################################################
...
class WindowVarLayer(nn.Module):
    def __init__(self, in_channels, h, w, groups=4, window_size=1):
        super(WindowVarLayer, self).__init__()

        self.gr = groups
        self.window_size = window_size
        self.channels_count = self.window_size * 4
        self.h = h
        self.w = w
        self.grad = nn.Sequential(
                nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1),
        ...
        for i in range(num):
            ...
            for direction in range(0, 4):
                ...
                for d in range(1, self.window_size+1):
                    index = (d-1)*4 + direction
                    if direction == 0 and self.valid(x,y+d):
                        a[i,index,i] = 1.0
                        a[i,index,self.convert(x,y+d)] = -1.0
                    
                    if direction == 1 and self.valid(x+d,y+d):
                        a[i,index,i] = 1.0
                        a[i,index,self.convert(x+d,y+d)] = -1.0
                    
                    if direction == 2 and self.valid(x+d,y):
                        a[i,index,i] = 1.0
                        a[i,index,self.convert(x+d,y)] = -1.0
                    
                    if direction == 3 and self.valid(x-d,y+d):
                        a[i,index,i] = 1.0
                        a[i,index,self.convert(x-d,y+d)] = -1.0
                    if i==(num-1):
                        a[i,index,i] = 1.0
...
class WindowVADepthNet(nn.Module):
    def __init__(self, pretrained=None, max_depth=10.0, prior_mean=1.54, si_lambda=0.85, img_size=(480, 640), swin_type = "tiny", window_size_directions=1):
        super().__init__()
        ...
        self.max_depth = max_depth
        self.swin_type = swin_type
        self.window_size_directions = window_size_directions
        ...
        if self.swin_type == "large":
            self.vlayer32 = WindowVarLayer(1536, img_size[0]//32, img_size[1]//32, groups=16, window_size=window_size_directions)
        else:
            self.vlayer32 = WindowVarLayer(1536//2, img_size[0]//32, img_size[1]//32, groups=16, window_size=window_size_directions)
        self.vlayer16 = WindowVarLayer(512, img_size[0]//16, img_size[1]//16, groups=16, window_size=window_size_directions)
        ...
        if self.swin_type == "large":
            self.var_loss_5 = WindowVarLoss(128, 1536, window_size=window_size_directions)
        else:
            self.var_loss_5 = WindowVarLoss(128, 1536//2, window_size=window_size_directions)
        self.var_loss_4 = WindowVarLoss(128, 512, window_size=window_size_directions)
        self.si_loss = SILogLoss(self.SI_loss_lambda, self.max_depth)
